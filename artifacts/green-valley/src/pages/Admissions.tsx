...
const Admissions = () => {
  ...
  return (
    <>
      ...
      <section id="process" className="container-prose py-20">
        <SectionHeader eyebrow="How it Works" title="Four steps to admission" />
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((s, i) => (
            <div key={s.title} className="shimmer-card group relative rounded-2xl border border-border bg-card p-6 shadow-card transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_0_0_2px_hsl(var(--primary)/0.2),0_16px_48px_-12px_hsl(var(--primary)/0.25)] hover:border-primary/50">
              <div className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl bg-primary text-primary-foreground transition-all duration-300 group-hover:scale-115 group-hover:rotate-6 group-hover:shadow-[0_0_18px_hsl(var(--primary)/0.5)]">
                <s.icon className="h-5 w-5" />
              </div>
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground transition-colors duration-300 group-hover:text-primary/60">
                Step {i + 1}
              </div>
              <h4 className="mt-1 font-serif text-xl transition-colors duration-300 group-hover:text-primary">{s.title}</h4>
              <p className="mt-2 text-sm text-muted-foreground transition-colors duration-300 group-hover:text-foreground/70">{s.text}</p>
            </div>
          ))}
        </div>
      </section>

      <section id="fee-structure" className="bg-gradient-leaf py-20">
      ...
      <section id="inquiry" className="container-prose py-20">
        ...
        >
          ...
          <Button
            ...
            size="lg"
            disabled={submitting}
            className="mt-8 w-full bg-primary hover:bg-primary-glow hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 md:w-auto"
          >
